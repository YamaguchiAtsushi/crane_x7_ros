...
def main():
    ...
    """
    target_pose = geometry_msgs.msg.Pose() #arm angle
    target_pose.position.x = 0.21
    target_pose.position.y = 0.45
    target_pose.position.z = 0.17
    q = quaternion_from_euler( -3.14, 0.0, 0.0 ) #(ai,aj,ak)=(roll,pitch,yaw)
    ...
    q = quaternion_from_euler(0.0, 0.0, 3.14/4.0)  # 上方から掴みに行く場合
    target_pose.orientation.x = q[0]
    target_pose.orientation.y = q[1]
    target_pose.orientation.z = q[2]
    target_pose.orientation.w = q[3]
    ...
    target_pose.position.y = -0.2
    target_pose.position.z = 0.2
    q = quaternion_from_euler( 0.0, 0.0, 0.0 ) #(ai,aj,ak)=(roll,pitch,yaw)
    target_pose.orientation.x = q[0]
    target_pose.orientation.y = q[1]
    target_pose.orientation.z = q[2]
    target_pose.orientation.w = q[3]
    ...
    q = quaternion_from_euler( -3.14/4, 0.0, 0.0 ) #(ai,aj,ak)=(roll,pitch,yaw)
    target_pose.orientation.x = q[0]
    target_pose.orientation.y = q[1]
    target_pose.orientation.z = q[2]
    target_pose.orientation.w = q[3]
